...
Default release suffix above is a Python f-string, the _dot_ divider between
`{sanitized_current_branch}` and `{git_desc_suffix}` is part of the _git_
_describe suffix_ (which is not included, if an error occurs when running
`git describe`).
